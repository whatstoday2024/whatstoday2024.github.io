          ...
          <div class="w-100">
            <VForm ref="form" class="card rounded-4 p-4 p-lg-5" v-slot="{ errors }" @submit="sendEmail">
              ...
              <div class="form-floating mb-4">
                <VField type="text" id="name" placeholder="請輸入姓名" name="姓名" class="form-control"
                  :class="{ 'is-invalid': errors['姓名'] }" rules="required" v-model="issue.name" />
                <ErrorMessage name="姓名" class="invalid-feedback" />
                <label for="name">請輸入姓名</label>
              </div>
              <div class="form-floating mb-4">
                <VField type="email" id="email" placeholder="請輸入電子信箱" name="信箱" class="form-control"
                  :class="{ 'is-invalid': errors['信箱'] }" rules="email|required" v-model="issue.email" />
                <ErrorMessage name="信箱" class="invalid-feedback" />
                <label for="email">請輸入電子信箱</label>
              </div>
              <div class="form-floating mb-4">
                <VField as="textarea" id="content" style="height:200px" placeholder="請輸入問題內容" name="問題內容"
                  class="form-control" :class="{ 'is-invalid': errors['問題內容'] }" rules="required"
